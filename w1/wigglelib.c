 ...
 *   the AW binary format generated by the present program
...
void sxWiggleParse (WIGGLE *sx, int z1, int z2)
{
  ...
  switch (sx->in)
	...
	*aap = arrayHandleCreate (x2 - x1 + 1, WIGGLEPOINT, sx->h) ;
	...
	{
	  int a1, a2, b, b1, b2 ;
	  messcrash ("wiggleParseFile stepIn=%d != stepOut=%d, this situation is not tested : %s",stepIn, stepOut, aceInFileName (ai)) ;
	  for (x = x1 ; x <= x2 ; x++)
	    {
	      a1 = x * stepIn ;
	      a2 = x * stepIn + span - 1 ;
      ...
      while ((cp = aceInCard (ai)))
	{
	  ...
	  if (strstr (cp, "Step"))
	      ...
	      if ((cq=strstr (cp, "step=")))
		{
		  ...
		  cc = * cr ; *cr = 0 ;
		  stepIn = atoi (cq) ;
		  sx->out_step = stepOut = (sx->out_step  ? sx->out_step : stepIn) ;
		  *cr = cc ;
		  x0 = x0 + 1 - stepIn ;
		  span = stepIn ;
		}
	  ...
	  switch (sx->in)
	      ...
	      if (! aceInFloat (ai, &y))
		{
	      ...
	      dx = stepIn/2 > 1 ? stepIn/2 - 1 : 0 ;
	      inxx = 1 ; xx1[0] = x0 -dx ; xx2[0] = x2 = x0 + stepIn - 1 -dx ;
	      if (sx->targets && !sxRemap (sx, 0, map, x0, x2, &remap, xx1, xx2, &inxx))
		continue ;
	      x1 = xx1[0] ; x2 = xx2[0] ;
	      if ((sx->strand && x1 > x2) || (sx->antistrand && x1 < x2)) continue ;
  ...
  if (debug)
    {
      aceOutf (ao, "\nEnds  stranded\n") ;
      for (ii = 0, mp = arrp (mmm, ii, MPK) ; ii < imm ; mp++, ii++)
	aceOutf (ao, "%s\t%d\t%d\t%d\t%d\t%.1f\t%.0f\t%d\t%d\n"
		 , dictName (sx->remapDict, remap)
		 , mp->x1, mp->x2, mp->ln * step
		 , mp->yMax, mp->cover * 1.0/(mp->ln), 1.0 * mp->cover * step
		 , mp->level, mp->level 
		 ) ;
    }
...
void sxWiggleExport (WIGGLE *sx)
{
  ...
  int limits[] = {1, 2, 5, 10, 20, 50, 100, 200, 500, 1000, 2000, 5000, 10000, 20000, 50000, 100000, 200000, 500000, 1000000, 2000000, 5000000, 10000000, 20000000, -1} ;
...
Array sxGetWiggleZone (Array aa, const char *fNam, char *type, int *stepp, const char *chrom, int z1, int z2, AC_HANDLE h0)
{
  AC_HANDLE h = 0 ;
  WIGGLE *sx = 0 ;
  char *rtype = 0 ;
