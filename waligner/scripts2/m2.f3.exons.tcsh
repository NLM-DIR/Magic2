...
if (! -e tmp/X.$MAGIC/$chrom/f3.genome.done2) then
    ...
    bin/tacembly tmp/X.$MAGIC/$chrom <<EOF
      ...
      parse tmp/GeneDB/$species.goodProduct.ace.gz
      parse tmp/GeneDB/$species.transcripts.ace.gz
      parse tmp/METADATA/$MAGIC.$target.captured_genes.ace
      parse tmp/METADATA/$target.GENE.info.ace
...
foreach target ($Etargets)
  ...
  if (! -e $ffcds) then
    zcat  GeneDB/*.[fr].cds_sponge.gz > GeneDB/$species.ns.cds_sponge
    set ffcds="GeneDB/$species.ns.cds_sponge"
  endif
  echo AAA $ffcds
  if (-e $ffcds) then
...
touch tmp/X.$MAGIC/$chrom/f3.done
endif
