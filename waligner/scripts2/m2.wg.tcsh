...
# 
  foreach uu (u nu pp)
	...
	ls -ls  tmp/SA/$run/wiggles/$run.$chrom.$uu.$fr.BF.gz
        if (-e  tmp/SA/$run/wiggles/$run.$chrom.$uu.$fr.BF.gz) then
          echo "tmp/SA/$run/wiggles/$run.$chrom.$uu.$fr.BF.gz " >>   $out/$fr.$uu.chrom.list
            set ok=1
	  endif
        endif
	  ...
	  if ($nr == 1) then
	    pushd $out
	      pwd
	      ls -ls ../../../SA/$run/wiggles/$run.$chrom.$uu.$fr.BF.gz
	      ln -s ../../../SA/$run/wiggles/$run.$chrom.$uu.$fr.BF.gz R.chrom.$uu.$fr.BF.gz
	    popd
	  endif
        endif
      endif
...
# evaluate the coverage per threshold
  ...
  if ($uu == u && ($fr == f || $fr == r) && -e  $out/R.chrom.$uu.$fr.BF.gz) then
    ...
    if (-e tmp/$WRG/$group/$chrom/R.chrom.u.ERF.BF.gz) then
      echo "Construct the transcriptsEnds  $WRG/$group"
      echo "  bin/wiggle  -transcriptsEnds tmp/$WRG/$group/$chrom/R.chrom.u -gzi -I BF -O COUNT -o tmp/$WRG/$group/$chrom/wg2a  -minCover 300 -wiggleRatioDamper 5"
              bin/wiggle  -transcriptsEnds tmp/$WRG/$group/$chrom/R.chrom.u -gzi -I BF -O COUNT -o tmp/$WRG/$group/$chrom/wg2a -minCover 300 -wiggleRatioDamper 5
    endif
endif
