...
When mapping to a human genome, the program requires about 25GB to 50Gb of memory. Depending on the hardware, the it will launch n threads (between 6 and 26 on current computers) and can process about 1 Gigabase of human RNA-seq reads per minute.

Adaptors are recognized and clipped automatically. 

In addition to the alignments, the program exports, at nearly no additional time cost, the coverage plots, the intron discovery and support, the gene expression indices, the location of the poly-A addition sites, the candidate start and ends of the transcripts, the details on the mismatches (substitution and indels with explict genome coordinates) and many detailled metrics.
...
```
sortalign -x IDX -i SRR24511885 -o results [--maxGB <int>] 
```
The --maxGB option will just download and analyze the first n gigabases of the SRA dataset.

Sortalign provides a convenient way to just download sequences from NCBI/SRA, 
sortalign         -sraDownlowd SRR1234,SRR5678  [--maxGB <int>] [--fastq] [--split-spots]

Or to download, cache the sequences and align at the same time
sortalign -x IDX -o results -i SRR1234,SRR5678  [--maxGB <int>] [--sraCaching]  [--fastq] [--split-spots]

In both cases, the reads are stored in a local directory called ./SRA. This is only useful if you need the reads several times, because sortalign starts aligning immediatly as soon as some data is received, so on a slow network, all the data will be aligned as soon as it has been fully transmitted.

The default format is .sra.fasta, each pair is presented in 4 consecutive lines: read 1 identifier, its sequence, read 2, its sequence. Requesting --fastq, the file will be called .sra.fastq and each pair occupies 8 consecutive lines (fastq read1 + fastq read2). Requesting --split-spots, read 1 and read 2 are stored in separate fasta or fastq files.  


### Output

The `-o` option specifies a path to the output directory. By default `sortalign` reports alignments in its internal .hits format. To request the SAM report, use `--sam` option. `--gzo` will gzip output files.

You can also generate target coverage wiggles in UCSC format with `--wiggles` and  `--wiggleEnds`

```
sortalign -x IDX -i SRR24511885 -o results --wiggles [--wiggleEnds]
```
