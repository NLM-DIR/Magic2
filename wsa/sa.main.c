...
int main (int argc, const char *argv[])
{
  ...
  if (getCmdLineInt (&argc, argv, "--testTorch", &bestGpu))
      return saTestTorch (bestGpu) ;
#endif
  ...
  if (! p.debug &&
      ...
      if (node >= 0 || ngpu > 0)   
	{
	  ...
	  if (ngpu > 0 && isExecutableInPath ("magic2_torch"))
	    {
	      char buf128[128] ;
	      snprintf (buf128, 128, "magic2_torch --testTorch %d", bestGpu) ;
	      int probe_rc = system (buf128) ;
	      if (probe_rc == 0)
		binary = "magic2_torch" ;
	      else
		fprintf (stderr,
...
#endif
#ifdef USE_TORCH
  if (saTestTorch (bestGpu >= 0 ? bestGpu : 0))
    messcrash ("magic2_torch failed while testing the GPU") ;
#endif
  ...
  if (1)
	...
	nSources++ ;
      channelSources (p.plChan, nSources) ;
