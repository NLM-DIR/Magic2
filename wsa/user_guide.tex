...
\begin{abstract}
  Magic2 is a long and short reads RNA deep-sequencing analyser.
  The program is easy to use because it analyses the first 10 mebabases of the data to select by itself the best alignemnt strategy and parameters.
  It is fast because in addition to the alignments, it exports the introns, the coverage plots and the gene expression profiles and other results in a single pass.
  The target, say a given version of the human genome, is constructed in a few minutes by providing a genome fasta file and the corresponding gff/gtf annotations.
  The reads can be private fasta/fastq files or downloaded on the fly from the NCBI short read archive (SRA) by just providing a set of SRR identifiers.
  The program analyses the hardware to tune its level of parallelisation. For the human genome, it needs 32 Gb of RAM, and will already work on a 4 CPU machine,
  but it runs faster on a more CPUs and, if available, it will use a GPU.
  As a becnhmark, we have compared Magic2 to Magic-Blast, HISAT, Star, and Minimap on 20 different datasets. each of size 2 Gigabases.
  We chose 12 Illumina paired en samples, 5 PacBio, 4 Nanopore, and 2 Roche runs. 17 are RNA-seq and 3 are DNA-seq. 12 are human samples, 6 are
  from different primates aligned to the T2T human genome, one is C.elegans and 3 are artificial baruzzo controls.
  A truth control is provided by also aligning the RefSeq full length mRNAs
  We ran all programs on the NCBI compute farm and measured the elapsed time, the CPU time, and percentages or pairs. reads and bases aligned ; the number
  of perfect reads and of compatible pairs ; and the number of mismatches and ambiguous alignments. Magic2 outperforms all the other codes
  on nearly all metrics, while exporting more results at no additional cost (alignments, expression, coverage plots, etc).
  The code is open-source and freely-available from git@github.com:NLM-DIR/Magic2.
...
accuratelly aligned an ever greater number of short piared-end Illumina reads
...
the presence of sequencing noise. To simulate a high noise level, we aligned
primatesequences, from Chimpanzee to Marmoset against the T2T human genome.
As a truth control for accurate splicing, we aligned the full length RefSeq mRNAs
and a contrario several DNA sample which a priori  should not skip introns.
...
Each object in the database has a name, belongs to a class
and is time-stamped to the last time a value was modified.
These fields can be accessed as methods
\BL
  select p in ?Paper, p.name, p.class, p.timestamp
\end{lstlisting}
