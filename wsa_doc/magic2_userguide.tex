...
\begin{titlepage}
\vspace*{20mm}
{\Huge \textbf{Magic2 User Guide}}\\[4mm]
{\Large A deep RNA-Sequencing Analyser}\\[2mm]
\vspace{8mm}
\hrule
\vspace{8mm}
{\large Jean ~Thierry-Mieg, Danielle ~Thierry-Mieg, Greg ~Boratyn}\\[2mm]
DIR and NCBI, National Library of Medicine, National Institutes of Health\\
...
Magic2 is an accurate, sensitive, RNA deep-sequencing analyzer designed for
...
counts, intron support tables, poly-A addition sites, coverage plots,
transcript-ends candidates and
...
\item Automatically recognises adaptor
  sequences, sequencing technology, and read length---no strategy
  parameters required.
\item Handles paired-end reads presented in separate files, or
  preferably in a single interleaved file.
\item Exports alignments in SAM, BAM, Magic \code{.hits}, or tabular format.
\item Adapts its parallelism to the available hardware--- no thread count
  parameter needed.
\item May offload on request part of the analysis to a GPU when one is present,
  although the performance gains are so far marginal.
\end{itemize}
...
Disk             & Varies with genome size; \(\sim\)22 GB for the human index \\
...
\subsection{Example 1: Align reads against a raw genome (not recommended)}
...
\subsection{Example 2: Align reads against an annotated genome (recommended)}

Three steps (see the details in Section~\ref{sec:targetIndex}):
...
This option accepts exactly one file or one paired-end pair.
...
To be automatically recognized, the file should be called *.sample\_12.fast[aq](.gz).
If not, use --interleaved on the command line, or write interleaved in the third column
of the read manifest.
...
\begin{notebox}
\textbf{Note:} Run label and adaptor sequences may be supplied as command-line flags
(\code{--adaptor1}, \code{--adaptor2}, \code{--run}).
These user-provided flags override the automatic recognition system.
When \code{-I} is used (see below), values given on the command line
can themselves be overridden run by run using optional column 3 of the run manifest file.
\end{notebox}
...
\midrule
1 & Filename & SRR identifier or path to a FASTA or FASTQ file, plain or gzip-compressed.
               For paired-end reads in two files, join the two paths with
               \code{+}, e.g.\ \code{R1.fastq.gz+R2.fastq.gz}. \\
2 & Run name & A nickname describing the sample.
...
The second column, the run name, allows one to choose
a meaningful sample name and to associate one or several data files to
the same sample. This is useful for example to group technical replicates (as opposed
to biological replicates). The program will export a single output table
(alignments, gene counts, intron counts, coverage plots...) per run.
By default, if column 2 is empty, the SRR identifier
or the file name stripped of its path information and suffix will be used as a run name.
(file=a/b/c.fasta.gz  is reduced to run=c).
...
would cause errors and will be rejected, because run names are used directly as components of
...
The third column is optional as the properties can in general be auto-detected.
By default, the file format is implied by the file suffix: .fasta, .fastq (or
.fastc for backward compatibility with MagicBlast).
The files called .sample\_12.fast[aq] are interleaved
(consecutive records ending in \code{/1} and \code{/2}).
The nature of the data: DNA or RNA, and any adaptors are recognized automatically
by the code by performing a dry run on the first 10 Megabases.
Thus, the third column is only needed to impose a choice and override the auto-detection.
...
\code{fasta}                 & Treat the file as FASTA. \\
\code{fastq}                 & Treat the file as FASTQ. \\
\code{fastc}                 & Treat the file as FASTC. \\
\code{interleaved}           & Treat the file as interleaved paired-end. \\
...
whereas \code{sample\_B} and \code{sample\_C} are independent.
The adaptors will be auto-detected in samples A and B, imposed in C.
RNA/DNA will be auto-detected in A, imposed in B and C. 
All three runs are processed simultaneously.


A manifest with only a filename column is also valid.  Magic2 will infer the
...
defaults to the filename without path or extension: sample\_A, sample\_B and sample\_C.
...
Streaming from SRA distinguishes single-end from paired-end automatically and begins
aligning as soon as the first megabases arrive.  In practice, alignment
finishes within seconds of completing the download.

\begin{notebox}
\textbf{Note:} If the files already exist in the local \code{./SRA/}
cache directory (for example created by previous \code{--sraDownload}), they are used
...
\begin{lstlisting}
magic2 -x IDX --srr SRR1,SRR2,SRR3 --no_ali --maxGB 1 -o out/
\end{lstlisting}
The option '\code{--maxGB 1}' restricts the analysis to the first gigabase of each run,
this is usually sufficient to assess the QC.
...
\textbf{Default download format: interleaved FASTA (recommended)}
\begin{lstlisting}
...
\code{--split\_pairs} & Write paired-end reads to separate \code{\_R1} and
                        \code{\_R2} files instead of interleaved (slower, not recommended). \\
\code{--maxGB <n>}   & Stop after downloading \code{n} gigabases. \code{n} is decimal (n=0.01 => 10Mb) 
                        By default the entire run is downloaded. \\
\bottomrule
\end{tabular}
\end{center}

Single-end runs are automatically exported as single files.
The number of SRR identifiers in a single
...
construct a run manifest \code{-I r.iConfig} as described in (Section~\ref{sec:rconfig}).
Data packets from all the runs will be processed in parallel, each run exporting its own
results in the global \code{-o outDir} directory. If all runs are streamed from SRA,
you can just list them on the command line \code {magic2 --srr SRR1234,SRR4567,...} 
as shown in (Section~\ref{sec:qcsurvey}).

\subsection{Processing multiple sets of runs in parallel on a multicore machine}

On a large multi-core Linux machine (such machines often have 48, or 96 or even more CPUs),
Magic selects the least busy core, and runs just on that one core. The purpose is to
eliminate data traffic across cores, which would significantly affect the performance.
But this means that on a 4-core machine, with say 96 CPUs, 3 cores remain idle.
As the index is memory-mapped and shared between
concurrent Magic2 processes, you may want on a 4-core machine to run 4 sets of runs in parallel
using the script:
\begin{lstlisting}
...
automatically select a different (multi-CPU) core.

To know the number of cores and CPUs available on your machine, type the
command:
...
\subsection{Overview}
\label{sec:targetIndex}

Before aligning any reads you must
...
same type are allowed and merged. Example:
...
\code{R} & Ribosomal RNA
  & rRNA precursor sequences.  \textbf{Strongly recommended}: rRNA can
    ...
    This fasta file should contain an examplar of all rRNA precursors.\\
\code{E} & Controls
...
used in the \code{A} annotation file.
\end{notebox}
...
The chromosome identifiers, column 1 of the gtf/gff files, must match the
chromosome identifiers given in the \code{G} genome fasta files.
...
The introns declared in the gtf/gff or intron file will be listed as known
in the results files, the others as novel.

\subsection{Creating the index}

\textbf{From a raw genome (FASTA only, for tests):}
\begin{lstlisting}
magic2 --createIndex IDX.genome -t genome.fasta.gz
\end{lstlisting}

\textbf{From a full configuration file (recommended):}
\begin{lstlisting}
...
\code{--step}        & int  & 6
                     & Seed stepping interval.  The best seed among the next
                       ...
                       \code{--maxtargetRepeats}  &int & 81 & Seeds repeated
                       more often in the genome are not indexed,
                       effectively masking the highly repetitive regions.\\                        
\bottomrule
\end{tabular}
\end{center}
Step is the maximal distance between seeds; the average distance is step/2.
...
\code{--hits}    & Magic \code{.hits} format.  Describes mismatches,
                   introns, read overhangs, and short indels explicitly. Default. \\
\code{--tabular} & Magic-BLAST tabular format.  Similar to \code{.hits}\\
\code{--sam}     & Standard SAM format.  \\
\code{--bam}     & Standard BAM format.  Required for tools such as
                   IGV, Samtools, and many genome browsers.\\
...
written to the output directory. By default, Magic2 exports in the
.hits format which is the richest.
...
\midrule
\code{*.hits}         & Magic hits      & Magic1 format, with full mismatches, overhangs and intron detail. \\
...
The \code{.hits} and tabular formats provide more detail than SAM/BAM,
...
third-party tools.
Magic2 exports natively in SAM format. SAM is then transformed into BAM
by automatically calling samtools. If samtools is not available on your
path, asking for \code{--bam} will revert to SAM format.

\subsection{Intron support table}

Magic2 reports all introns observed in the alignment, classified as
\emph{known} (present in the \code{A} annotation used during index
construction) or \emph{novel}.

Output is in the \textbf{tag-sample format (TSF)}, a compact
tab-separated format in which each line records the support of a given intron in a given run.
...
strand, GT-AG motif support, and read count per run. Example:
...
Coverage plots are exported by default with one value every 10 bases.
A different resolution can be chosen using --wiggle\_step.
Notice that the same total numbe rof bases is reported in both cases,
say $(1,2,2,2,4,3,3,0,0,0)$ at resolution 1 would give $(17)$ at resolution 10.
By default, magic2 export using the UCSC broweser compatible BF format
(one value per line). One can alternatively request the binary AZ format
where data are split with one binary file every 2 Megabases. This format
strongly accelerates the acedb graphic browser but is otherwise not
interoperable. One can also request the UCSC bigwig format. The bw exportation
is slower (tens of minutes rather than 10s of seconds) and will only work
if the UCSC executable code wigToBigWig is available on
your machine (on your PATH), otherwise magic2 falls back onto BF.gz

\begin{lstlisting}
magic2 -x IDX -i reads.fasta.gz -o out/ --wiggles
magic2 -x IDX -i reads.fasta.gz -o out/ --wiggles --wiggle_step 20 --bigWig
\end{lstlisting}
...
\code{--BF} &[Default] UCSC BF.gz format\\
\code{--AZ} &Internal binary format\\
\code{--bigWig} &UCSC bigWig format\\
\bottomrule
\end{tabular}
\end{center}
...
Magic2 also exports a track of non-unique alignments, showing the
expressed repetitive regions, and a track of incomplete alignments indicating
...
Aligned\_pairs & Number and fraction of aligned pairs. \\
Compatible\_pairs & Number and fraction of compatible aligned pairs. \\
Circle\_pairs & Number and fraction of locally diverging pairs (candidate circles). \\
Non\_compatible\_pairs  & Number and fraction of non-compatible aligned pairs. \\
Reads & Total number of raw reads, counting 2 reads per pair if applicable. \\
Low\_entropy\_reads & Number of rejected reads with very low sequence complexity. \\
Low\_entropy\_bases & Number of bases in those reads. \\
Aligned\_reads & Number of aligned reads. \\
Unaligned\_reads & Number of unaligned reads. \\
Perfect\_reads & Number and fraction of fully and exactly aligned reads. \\
Complex\_reads & Number and fraction of reads aligned in an unusual way. \\
Partial\_reads & Number and fraction of partially aligned reads. \\
Bases & Number of bases in the raw reads: total, in read 1, in read 2. \\
Aligned\_bases   & Number and fraction of aligned bases: total, in read 1, in read 2. \\
Unaligned\_bases  & Number of unaligned bases. \\
Genome\_length  & Cumulated length of the class G (genomic) targets. \\
CDS\_utr\_intronic\_intergenic\_Bases & Number and fraction of bases mapping in the different annotated regions. \\
ATGCN   & Number of bases A, T, G, C or other in the raw reads. \\
Clipped\_polyA & Number of reads with at least 8 overhanging terminal A. \\
Clipped\_polyT & Number of reads with at least 8 overhanging initial T. \\
polyA\_sites  & Number of observed polyA addition sites. \\
...
Mismatches\_and\_InDels & Number (and fraction of aligned bases) of substitutions and short indels. \\
Strategy   & A positive number in RNA samples, negative in DNA samples. \\ 
Intron\_supports & Number of local read supports for new and novel introns. \\
Supported\_introns & Number of new and novel supported introns.\\
Min\_read\_length & Minimal read length. \\
Max\_read\_length & Maximal read length. \\
Length\_distribution\_1\_5\_50\_95\_99\_mode\_av  & Read length distribution per quantile. \\
Reads\_Aligned\_once & Number and Fraction of all reads uniquely mapped. \\
Reads\_multi\_aligned\_\_n & Number and Fraction of all reads mapped 2,3,4...10 times. \\
Reads\_aligned\_several\_times & Number and Fraction of all reads mapped several times. \\
Reads\_aligned\_in\_class\_X & Number and Fraction of aligned reads per strand per class (rRNA, Genome...). \\
Bases\_aligned\_in\_class\_X & Number and Fraction of aligned bases per strand per class (rRNA, Genome...). \\
\bottomrule
\end{longtable}
\end{center}
...
\begin{lstlisting}
cat out_dir/*/runStats.tsf | gawk -F '\t' '{if($2 == tag)print;}' tag=Perfect  | bin/tsf -I tsf -O table --title 'Perfect reads' > perfect_reads.table.txt
\end{lstlisting}
...
\code{--sam}          & ---           & off      & Export in SAM format. \\
...
Index creation fails with out-of-memory error
  & Increase available RAM, or use a larger \code{--step} value (e.g.\
    8 or 10) to reduce index size at some cost to sensitivity. \\[4pt]
...
Chromosome names don't match between genome and GTF
  & This is a fatal error reported during index creation.
    ...
    exactly match those in the \code{A} annotation file (including
    \code{chr} prefix vs.\ no prefix). \\[4pt]

BAM file not produced despite \code{--bam} flag
& To transform a SAM output into a BAM output, Magic2 invokes 'samtools'.
Otherwise Magic2 reverts to SAM format.
Samtools is not part of the Magic2 distribution. You must install it from the web
and make it accessible. Check by typing: which samtools.\\[4pt]

Missing gene counts output despite \code{--full}
  & Confirm that the index was built with \code{-T} (tConfig including
ydd    an \code{A} annotation line) rather than \code{-t} (raw genome only).
