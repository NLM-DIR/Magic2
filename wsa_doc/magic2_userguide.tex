...
\item May offload on request part of the analysis to a GPU when one is present,
  although the performace gains are so far marginal.
\end{itemize}
...
can them selves be overriden run by run using optional column 3 of the run manifest file.
\end{notebox}
...
The second column, the run name, allows to choose
a meaningful sample name and to associate to one or several data files to
the same sample. This is useful for example to group technical replicas (as opposed
to biological replicas). The program will export a single output table
(alignments, gene counts, intron counts, coverage plots...) per run.
By default, if column 2 is empty, the SRR identifer
...
The nature of the data: DNA or RNA, and the eventual adaptors are recognized automatically
...
Here is an exampl with 5 files and 3 run names:
...
finishes within seconds of the completeing the download.
...
this is usually sufficient to asees the QC.
...
\subsection{Processing multiple sets runs in parallel on a multicore machine}

On a large multi-core linux machine (such machine often have 48, or 96 or even more CPUs),
Magic selects the least buzy core, and runs just on that one core. The purpose is to
eliminate data traffic accross cores, which would significantly affect the performances.
...
chromosome identifiers given the the \code{G} genome fasta files.
...
In introns declared in the gtf/gff or intron file will be listed as known
in the results files, the other as novel.
...
\code{--step}        & int  & 6
                     & Seed stepping interval.  The best seed among the next
                       ...
                       effectivelly masking the highly repetitive regions.\\                        
\bottomrule
\end{tabular}
\end{center}
...
\code{--hits}    & Magic \code{.hits} format.  Describes mismatches,
                   introns, read overhangs, short indels and mismatches explicitly. Default. \\
...
written to the output directory. By defalt, Magic2 exports in the
...
third-party tools.
Magic2 exports nativelly in SAM format. SAM is then transformed in BAm
by calling automatically samtools. If samtools is not available on your
...
Low\_entropy\_bases & Number of baes in those reads. \\
Aligned\_reads & Number of aligned reads. \\
Unligned\_reads & Number of unaligned reads. \\
...
Genome\_length  & Cumulated length of the calss G (genomic) targets. \\
...
Reads\_aligned\_several\_times & Number and Fraction of all reads mapped several time. \\
...
Otherwise Magic2 reverts to SAM fromat.
...
Missing gene counts output despite \code{--full}
  & Confirm that the index was built with \code{-T} (tConfig including
    a \code{I} annotation line) rather than \code{-t} (raw genome only).
